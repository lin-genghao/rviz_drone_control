...
SSLClient::verify_host_with_subject_alt_name(X509 *server_cert) const {
  ...
  if (alt_names) {
    ...
    for (decltype(count) i = 0; i < count && !dsn_matched; i++) {
      ...
      // jam add skip authentication host
      if (type == GEN_IPADD && val->type == GEN_DNS) {
        auto name = (const char *)ASN1_STRING_get0_data(val->d.ia5);
        auto name_len = (size_t)ASN1_STRING_length(val->d.ia5);
        if (std::string(name, name_len) == "master.local"
        || std::string(name, name_len) == "*.master.local") {
          ip_mached = true;
          break;
        }
      }
      // end skip authentication host
      if (val->type == type) {
        auto name = (const char *)ASN1_STRING_get0_data(val->d.ia5);
        auto name_len = (size_t)ASN1_STRING_length(val->d.ia5);
